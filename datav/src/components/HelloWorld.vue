<template>
  <div class="hello" :style="wrapStyle">
    ...
    >
      <component :ref="item.name" :is="item.componentName" v-bind="item.componentOption">
      </component>
    </vue-draggable-resizable>
    <!-- 对齐基线 -->
    <span
...
import ThreedTags from "./threed-tags";
export default {
  name: "HelloWorld",
  props: {
    msg: String,
  ...
  methods: {
    ...
    getBaseOption(item) {
      return {
        ...item,
        w: item.w === "100%" ? document.body.offsetWidth : item.w,
        h: item.h === "100%" ? document.body.offsetHeight : item.h,
      };
    },
  ...
  created() {
    this.addLayer(config[0]);
    this.addLayer(config[1]);
    // this.addLayer(config[2]);
  },
};
</script>
